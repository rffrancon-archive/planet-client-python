...
def read(value):
    value = value or '@-'
    if value[0] == '@':
        with click.open_file(value[1:]) as fp:
            if not fp.isatty():
                value = fp.read()
            else:
                value = None
...
def build_conditions(workspace):
    ...
    aoi = json.dumps(api.utils.feature_collection(aoi))

    keys = set(filters.keys())
    keys.remove('geometry')
    # for now - need to convert to labels ['test', 'standard', 'target']
    keys.remove('image_statistics.image_quality')
    for key in keys:
        rules = filters[key]
        conditions.update([
...
def get_scenes_list(scene_type, pretty, aoi, count, where, workspace):
    ...
    if workspace:
        ...
        ws_filters = build_conditions(workspace)
        if aoi:
            ws_filters.pop('geometry', None)
...
@cli.command('sync')
def sync(destination, workspace, scene_type, limit):
    ...
    if workspace:
        workspace = call_and_wrap(client().get_workspace, workspace).get()
        filters = build_conditions(workspace)
    ...
    sync_tool.sync(progress_callback)

    if sync_tool.transferred:
        summarize_throughput(sync_tool.transferred, start_time)
